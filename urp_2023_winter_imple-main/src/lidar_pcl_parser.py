...
class LiDARProcessing():
    ...
    def callback(self, data):
        ...
        # 차선 안쪽 차 앞 장애물만 인식하도록 차 앞 직사각형 부분만 가져오게 잘라 놓음.
        # RANSAC(겁나 오래걸림), Z자르기
    
        
        cloud = do_passthrough(cloud, 'x', 0, 10)
        cloud = do_passthrough(cloud, 'y', -1.5, 1.5)
        cloud = do_passthrough(cloud, 'z', -0.6, 2)        
        cloud_new = lidar_pcl_helper.pcl_to_ros(cloud)
        
        #ransac nono
        # _, _, cloud_new= do_ransac_plane_normal_segmentation(cloud_new,0.15)
        # cloud_new = lidar_pcl_helper.pcl_to_ros(cloud_new)
